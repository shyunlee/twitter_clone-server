...
describe("Auth API Test", () => {
  ...
  describe("POST to /auth/login", () => {
    it("returns 200 status when logged in successfully", async () => {
      ...
      const response = await httpRequest.post('/auth/login', {username: newUser.username, password: newUser.password})
    ...
    ])('returns 401 status when login is failed with wrong $wrongData', async ({inputData, expectedMessage}) => {
      const newUser = await createNewUser()
    ...
    test.each([
      {inputData: 'username', expectedMessage:'username should be at least 5 characters'},
      {inputData: 'password', expectedMessage: 'password should be at least 5 characters'}
    ])('returns 400 when validation is failed due to $inputData', async ({inputData, expectedMessage}) => {
      const newUser = await createNewUser()
  ...
  describe('GET to /auth/me', () => {
    it('returns 200 status when token in Authorization header is valid', async () => {
      ...
      expect(response.status).toBe(200)
      expect(response.data).toMatchObject({username: userRegistered.username, userId: userRegistered.userId, token: userRegistered.jwt})
    })
